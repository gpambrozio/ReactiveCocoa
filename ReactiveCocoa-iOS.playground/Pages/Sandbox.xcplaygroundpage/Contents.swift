...
class Exercise34ViewController : UIViewController {
    ...
    override func loadView() {
        ...
        buttonYes = UIButton(type: .system)
        buttonYes.setTitle("Yes", for: .normal)
        buttonNo = UIButton(type: .system)
        buttonNo.setTitle("No", for: .normal)
        buttonMaybe = UIButton(type: .system)
        buttonMaybe.setTitle("Maybe", for: .normal)

        errorLabel = UILabel()

        let stack = UIStackView(arrangedSubviews: [buttonYes, buttonNo, buttonMaybe, errorLabel])
